...
- [x] Step 2 - 전화번호 입력
- [x] Step 2 - 이름을 입력한 경우에만 화면에 뜬 연락처 이름 검증
...
python google_message.py "홍길동" "+821012345678" "테스트 메시지입니다."
```
...
7. 이름을 입력한 경우에만 연락처 목록에 뜬 이름이 입력한 이름과 정확히 같은지 확인합니다.
8. 이름이 다르거나 연락처 결과가 없으면 즉시 중단하고 Step 3/4는 실행하지 않습니다.
9. `~번으로 보내기` 버튼을 클릭하고 메시지 입력창을 확인합니다.
10. 메시지 인자가 없으면 실행 중 메시지를 입력받고, 메시지를 입력한 뒤 전송 버튼을 클릭합니다.
11. 메시지 입력창이 비워지면 전송 성공으로 종료합니다.
...
python google_message.py "홍길동" "+821012345678" --fill-only
```

## 전송 없이 멈추기

기본 동작은 메시지를 받아 전송합니다. 전송하지 않으려면 `--fill-only`로 Step 2에서 멈춥니다.
이전 버전처럼 메시지 인자 생략만으로 Step 3에서 멈추지는 않습니다.
...
python3 google_message.py "+821012345678" "테스트 메시지입니다." --attach-only
```
...
python google_message.py "+821012345678" "테스트 메시지입니다." --attach-only
```
...
- `--name` - 기대하는 연락처 이름 지정. 생략하면 이름 검증 없이 전화번호로 바로 진행
