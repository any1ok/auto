...
Step 8 — 열린 채팅방에 이미지/파일을 파일 URL paste 또는 파일 선택창 + Return 으로 전송
"""
...
def _set_clipboard_file(path: Path) -> None:
    """macOS pasteboard 에 텍스트 경로가 아니라 파일 URL 타입으로 1개 파일을 올린다."""
    escaped = _applescript_escape(str(path))
    _run_osascript(f'set the clipboard to (POSIX file "{escaped}")')
...
def _send_file_via_clipboard(chat_title: str, path: Path) -> str:
    """이미지처럼 file URL paste 로 정상 업로드되는 파일용 경로."""
    _focus_chat_window(chat_title)
    _set_clipboard_file(path)
    _run_osascript(
        f'tell application "System Events" to tell process "{KAKAO_APP_NAME}" '
        f"to key code {KEY_CODE_V} using command down"
    )
    ...
    return f"file-url-paste/{send_button}"


def _send_file_via_picker(chat_title: str, path: Path) -> str:
    """PDF 등 일반 파일용 경로. 파일 선택창은 열지만 전송 확정은 Return 만 쓴다."""
...
def step8_send_file(chat_title: str, file_path: str) -> None:
    ...
    검증된 흐름:
      - 이미지: file URL paste 로 카카오톡 '파일 전송' 확인 sheet 를 띄운다.
      - 일반 파일: 첨부 파일 선택창을 열어 경로를 선택한다.
      - 두 경우 모두 실제 전송 확정은 Return 으로만 한다.
    """
    path = _resolve_existing_file_path(str(file_path))
    if _is_image_file(path):
        method = _send_file_via_clipboard(chat_title, path)
    else:
